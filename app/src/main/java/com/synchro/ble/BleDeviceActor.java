...
public class BleDeviceActor implements Runnable {
    ...
    public static void connectToDevice() {
        ...
        String macAddress = SharedPref.getValue(mContext,SharedPref.CONNECTED_MAC_ADDRESS_PREF,"");
...
//                    bleCallback.connectionCallback(false, " Connection error: Device went out of range");
                ...
                Log.d(AppConstants.TAG, "disconnect callback");
                reconnectDevice();
            } else {
                isConnected = false;
                bleCallback.connectionCallback(false, " Connection error status:"+status);
    ...
    public static void reconnectDevice(){
        Fragment f = ((MainActivity)mContext).getSupportFragmentManager().findFragmentById(R.id.frameLayout);
        if (f instanceof ConnectDeviceFragment) {
    ...
    public void run() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                if (GlobalApplication.isCoded==1) {
                    scanSettingsBuilder.setLegacy(false);
                    scanSettingsBuilder.setPhy(ScanSettings.CALLBACK_TYPE_ALL_MATCHES);
                }
            }
            List<ScanFilter> filters = new ArrayList<ScanFilter>();
          /*  ScanFilter filter = new ScanFilter.Builder()
                    .setServiceUuid(new ParcelUuid(UUID.fromString(BleConstant.DATA_SYNC_SERVICE_UUID)))
                    .build();
            filters.add(filter);*/
            if (btScanner != null) {
    ...
    private android.bluetooth.le.ScanCallback ScanCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
           ...
           /* if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                Log.d(AppConstants.TAG, "primary:"+result.getPrimaryPhy()+" secondary:"+result.getSecondaryPhy());
            }*/
...
//                    Log.d(AppConstants.TAG, "onScanResult: " + scannedDevice.getAddress() + ", name:" + deviceName);
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ...
                    if (deviceName.toLowerCase().equals(AppConstants.filter_devicename)) {
                        bleCallback.scanCallback(deviceName, scannedDevice.getAddress(), isCoded(result));
                    } else if (deviceName.toLowerCase().startsWith(AppConstants.filter_devicename_start)) {
    ...
    private String isCoded(ScanResult result) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (result.getPrimaryPhy() == BluetoothDevice.PHY_LE_CODED || result.getPrimaryPhy() == BluetoothDevice.PHY_LE_CODED_MASK || result.getSecondaryPhy() == BluetoothDevice.PHY_LE_CODED || result.getPrimaryPhy() == BluetoothDevice.PHY_LE_CODED_MASK) {
                return "Long";
            }
        }
        return "Short";
